...
float4 value = read_imagef(input, sampler, position);
...
fn main() -> Result<()> {
    ...
    // Create images
    let mut image_input_data = rainbow_image(10, 10);
    let mut image_input = unsafe {
        Image::create(
            &context,
            CL_MEM_WRITE_ONLY,
            &cl_image_format {
            ...
            image_input_data.as_mut_ptr() as *mut c_void,
        )
        .expect("Image::create failed")
    };

    let mut image_output = unsafe {
        Image::create(
            &context,
            CL_MEM_WRITE_ONLY,
            &cl_image_format {
    ...
    let mut image_output_data = [0u8; 10 * 10 * 4];
    let read_event = unsafe {
        queue.enqueue_read_image(
            ...
            0,
            0,
            image_output_data.as_mut_ptr() as *mut c_void,
            &events,
        )?
    };
    ...
    for y in 0..10 {
        for x in 0..10 {
            ...
            print!(
                "({:>3}, {:>3}, {:>3}, {:>3}) ",
                image_output_data[offset],
                image_output_data[offset + 1],
                image_output_data[offset + 2],
                image_output_data[offset + 3]
            );
        }
        println!();
    }
...
fn rainbow_image(width: usize, height: usize) -> Vec<(u8, u8, u8, u8)> {
    let width: usize = 10;
    let height = 10;
    let mut image_data: Vec<(u8, u8, u8, u8)> = Vec::with_capacity(width * height);
    for y in 0..height {
        for x in 0..width {
            let ratio = x as f32 / width as f32;
            let (r, g, b) = rainbow_color(ratio);
            image_data.push((r, g, b, 255)); // Alpha value is 255 (fully opaque)
        }
    }
...
fn rainbow_color(ratio: f32) -> (u8, u8, u8) {
    // Convert the ratio to a color in the rainbow
    let hue = ratio * 360.0;
